...
// This structure holds cached information used by the algorithm. Two
// model pointers and two colliding primitives are cached. Model
// pointers are assigned to their respective meshes, and the pair of
// colliding primitives is used for temporal coherence. That is, in
// case temporal coherence is enabled, those two primitives are tested
// for overlap before everything else. If they still collide, we're
// done before even entering the recursive collision code.
...
public:
   ...
   *	\param		world0		[in] world matrix for first object, or 0
   *	\param		world1		[in] world matrix for second object, or 0
   *	\return		true if success
   *	\warning	SCALE NOT SUPPORTED. The matrices must contain rotation & translation parts only.
   */
  //----------------------------------------------------------------------
  bool Collide(BVTCache& cache, const Matrix4x4* world0=0, const Matrix4x4* world1=0);

  // Collision queries
  bool Collide(const AABBCollisionTree* tree0, const AABBCollisionTree* tree1,
               const Matrix4x4* world0=0, const Matrix4x4* world1=0, Pair* cache=0);
  ...
  //----------------------------------------------------------------------
  /**
   *	Settings: selects between full box-box tests or "SAT-lite"
   *	tests (where Class III axes are discarded)
  ...
  //----------------------------------------------------------------------
  /**
   *	Settings: selects between full triangle-box tests or
   *	"SAT-lite" tests (where Class III axes are discarded)
   *	\param	flag	[in] true for full tests, false for coarse tests
   ...
   */
  //----------------------------------------------------------------------
  udword GetNbBVBVTests() const { return mNbBVBVTests; }

  //----------------------------------------------------------------------
  /**
   * Stats: gets the number of Triangle-Triangle overlap tests
   * after a collision query.
   * \see     GetNbBVBVTests()
   * \see     GetNbBVPrimTests()
   ...
   */
  //----------------------------------------------------------------------
  udword GetNbBVPrimTests() const { return mNbBVPrimTests; }
   ...
   */
  //----------------------------------------------------------------------
  udword GetNbPairs() const { return mPairs.GetNbEntries()>>1; }
   ...
   *	all the settings and callbacks have been defined for a
   *	collider.
...
protected:
  ...
  // Init methods
  void	InitQuery(const Matrix4x4* world0=0, const Matrix4x4* world1=0);
  bool	CheckTemporalCoherence(Pair* cache);

  bool	Setup(const MeshInterface* mi0, const MeshInterface* mi1)
  {
    mIMesh0 = mi0;
    mIMesh1 = mi1;
