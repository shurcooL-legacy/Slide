...
public:
  ...
  Plane(float nx, float ny, float nz, float d)  { Set(nx, ny, nz, d); }
  //! Constructor from a point on the plane and a normal
  Plane(const Point& p, const Point& n)         { Set(p, n); }
  //! Constructor from three points
  Plane(const Point& p0, const Point& p1, const Point& p2) { Set(p0, p1, p2); }
  //! Constructor from a normal and a distance
  Plane(const Point& _n, float _d)              { n = _n; d = _d; }
  //! Copy constructor
  Plane(const Plane& plane) : n(plane.n), d(plane.d) {}
  //! Destructor
  ~Plane() {}

  Plane& Zero()	{ n.Zero(); d = 0.0f; return *this; }
  Plane& Set(float nx, float ny, float nz, float _d) { n.Set(nx, ny, nz); d = _d; return *this; }
  Plane& Set(const Point& p, const Point& _n)        { n = _n; d = - p | _n; return *this; }
  Plane& Set(const Point& p0, const Point& p1, const Point& p2);

  float	Distance(const Point& p) const { return (p | n) + d; }
  bool	Belongs (const Point& p) const { return fabsf(Distance(p)) < PLANE_EPSILON; }
...
public:
  ...
  operator HPoint() const { return HPoint(n, d); }

  // Arithmetic operators
  Plane operator*(const Matrix4x4& m) const
  {
  ...
  Plane& operator*=(const Matrix4x4& m)
  {
    // Old code from Irion. Kept for reference.
...
//----------------------------------------------------------------------
/**
 *	Transforms a plane by a 4x4 matrix. Same as Plane * Matrix4x4
 *	operator, but faster.
