...
public:
  ...
  IndexedTriangle() {}
  //! Constructor
  IndexedTriangle(udword r0, udword r1, udword r2) { mVRef[0]=r0; mVRef[1]=r1; mVRef[2]=r2; }
  //! Copy constructor
  IndexedTriangle(const IndexedTriangle& triangle)
  {
  ...
  udword	OppositeVertex(ubyte edgenb) const { return mVRef[2-edgenb]; }
  ...
  void		ComputePoint(const Point* verts, float u, float v, Point& pt, udword* nearvtx=0) const;
  float		Angle(const IndexedTriangle& tri, const Point* verts) const;
  Plane         PlaneEquation(const Point* verts) const { return Plane(verts[mVRef[0]], verts[mVRef[1]], verts[mVRef[2]]); }
