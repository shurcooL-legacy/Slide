...
public:
  ...
  OBB()	{}
  //! Constructor
  OBB(const Point& center, const Point& extents, const Matrix3x3& rot) :
  ...
  const Point&		GetExtents()	const	{ return mExtents; }
  const Matrix3x3&	GetRot()	const	{ return mRot;	   }

  void	GetRotatedExtents(Matrix3x3& extents)	const
  {
    extents = mRot;
