...
public:
  ...
  bool	Collide(PlanesCache& cache, const Plane* planes, udword nb_planes,
                const Model& model, const Matrix4x4* worldm=0);

  // Mutant box-with-planes collision queries
  bool	Collide(PlanesCache& cache, const OBB& box, const Model& model,
                const Matrix4x4* worldb=0, const Matrix4x4* worldm=0)
  {
    Plane PL[6];
...
protected:
  ...
  // Overlap tests
  inline_ bool	PlanesAABBOverlap(const Point& center, const Point& extents,
...
public:
  ...
  bool	Collide(PlanesCache& cache, const Plane* planes, udword nb_planes,
                const HybridModel& model, const Matrix4x4* worldm=0);
