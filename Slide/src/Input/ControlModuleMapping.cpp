...
void ControlModuleMapping::ProcessAxis(InputManager::InputId AxisId, InputManager::AxisState AxisState)
{
...
void ControlModuleMapping::Process2Axes(InputManager::InputId FirstAxisId, InputManager::AxisState AxisState[2])
{
	auto Range = m_AxisMappings.equal_range(FirstAxisId);
	for (auto it0 = Range.first; it0 != Range.second; ++it0)
		it0->second.first->Process2Axes(it0->second.second, AxisState);

// In Slide, there are no active module changes due to axis events, only buttons
#if 0
	// Active Module change
	{
		auto NewActiveModule = m_ActiveModule;
...
else if (typeid(UnderCursorTranslationModule) == typeid(*ActiveModule)) printf("ChangeActiveModule(UnderCursorTranslationModule)\n");
else if (typeid(TwoAxisValuatorModule) == typeid(*ActiveModule)) printf("ChangeActiveModule(TwoAxisValuatorModule)\n");
