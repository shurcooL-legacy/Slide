		...
		if ((*it0)->GetId() == GetSelectedObjectId() || (GetSelectedObjectId() && SubSelectedObjects.end() != SubSelectedObjects.find((*it0)->GetId())))
		{
			...
			for (std::vector<SceneObject *>::iterator it1 = MyScene.m_Objects.begin(); it1 != MyScene.m_Objects.end(); ++it1)
			{
				...
				if (IsOk)
				{
					...
					{
						auto NbPairs = Collider.GetNbPairs();
						const auto pPairs = Collider.GetPairs();

						for (decltype(NbPairs) nPair = 0; nPair < NbPairs; ++nPair)
						{
		...
		if (m_ModelMovedByUser)
		{
			...
			// DEBUG: This needs to be redone properly
			{
				...
				if (!SelectedObjectVisible) {
					...
					NewSnapB(SnapMode::Front, StartingPosition::Invisible);
					printf("Brought it forward by one layer.\n");

					// DEBUG: This depends on how Snap-Forward works. If snap-forward only pushes the object closer by one layer,
					//		  we have to keep doing this every frame. If it makes it visible in one go, we can set m_ModelMovedByUser to false
			...
			if (!(true && 0 != m_SlidingObject && Wm5::Vector3d::ZERO != m_SlidingConstraint.Normal && !CollisionExists))
			{
				...
				if (!CollisionExists)
					...
					NewSnap(SnapMode::Back);
				else
					// Snap front
					//SnapOrtho(SnapMode::Front, StartingPosition::Colliding);
					NewSnapB(SnapMode::Front, StartingPosition::Colliding);
#else
				// Snap back
				if (!CollisionExists)
				{
...
void Slide::PickModelAndComputeUnderCursorPositionAndRayDirection(int16 PositionX, int16 PositionY)
{
//if (MyScene.GetSelectedObjectId()) MyScene.GetSelectedObject().m_oIntersectingTris.clear();
	...
	if (0 != NewSelectedObject && !MyScene.GetObject(NewSelectedObject).IsSelectable()) NewSelectedObject = 0;		// Make sure the object is selectable, else prevent it from getting selected
...
// Snap back, assuming no collision
void Slide::NewSnap(SnapMode SnapMode)
{
	Wm5::Vector3d SnapDirection;
	if (Back == SnapMode)
		SnapDirection = m_CursorRayDirection;		// Assuming m_CursorRayDirection is normalized
	...
	NewSnap(SnapMode, SnapOrigin, SnapDirection, StartingPosition::NonColliding);
}

void Slide::NewSnap(SnapMode SnapMode, Wm5::Vector3d SnapOrigin, Wm5::Vector3d SnapDirection, StartingPosition StartingPosition/*, bool SkipVisibilityCheck*/)
{
	double MoveAmount = 0;
	...
	const uint32 nCustomViewportWidth = MySlideTools->m_DepthPeelImageWidth;
	const uint32 nCustomViewportHeight = MySlideTools->m_DepthPeelImageHeight;
...

	{
		...
		for (uint8 DepthLayer = 0; DepthLayer <= 1; ++DepthLayer)
		{
			// Set target FBO
			...
			if (0 == DepthLayer)
			{
				...
				MyScene.Render(Scene::SelectedObjectFurthestBackFace, this);
			...
			else
			{
				...
				if (Back == SnapMode)
				{
					...
					if (2 == dMinPositiveDistance && true/*Invisible != StartingPosition*/) {
						break;
					}
					//////// ^^^^ THIS THING ABOVE NEEDS TO BE DONE PROPERLY

					{
						MoveAmount += 2 * m_kBufferDistance;		// Push it a little further *back* to force a collision, which will cause geometry to resolve
						MoveSelectedObject(MoveAmount * SnapDirection);
						...
						{
							double DontCare;
							auto ResolvedCollision = TryResolveCollisionGeometry(SnapDirection, DontCare);
...
//SlidingNormal = MyScene.GetSlidingPlane().Normal;
	...
	printf("SelObj.Z() = %.20f (%f, %f) - Snap-Back\n", GetSelectedObject().GetPosition().Z(), GetSelectedObject().GetPosition().X(), GetSelectedObject().GetPosition().Y());
...
// Pre-condition: Collision exists, collision information set
// Returns true if successfully resolved collision
// Returns false if failed
bool Slide::TryResolveCollisionGeometry(const Wm5::Vector3d SnapDirection, double & OutputMoveAmount)
{
	bool CollisionExists = true;		// Precondition

	// Save starting state
	auto StartingState1 = GetSelectedObject().GetPosition();
	auto StartingState2 = m_UnderCursorPosition;
	...
	while (CollisionExists && Iterations > 0)
	{
		...
		auto MoveAmount = SnapGeometry2(SnapDirection);
		OutputMoveAmount += MoveAmount;
		MoveSelectedObject(MoveAmount * SnapDirection);
	...
	if (CollisionExists)
	{
		//printf("Failed, took too many SnapGeometry2() iterations.\n");

		// Failed, restore starting state
		GetSelectedObject().ModifyPosition() = StartingState1;
		m_UnderCursorPosition = StartingState2;

		return false;
	}
	else
	{
		//printf("Succeeded in %d SnapGeometry2() iterations.\n", 3 - Iterations);

		return true;
	}
}

bool Slide::FindFirstValidPosition(MultiRange<float> & Positions, const Wm5::Vector3d SnapDirection)
{
	Wm5::Vector3d OriginalSelectedObjectPosition = GetSelectedObject().GetPosition();		// TODO: Make this function operate using temporary object positions, instead of physically moving selected object inside; after that, this variable is no longer needed

	// Iterates over positions and finds the first one that is geometrically valid
	while (!Positions.empty())
	{
		double MoveAmount = 100 * static_cast<double>(Positions.front().second);		// User upper bound
		Positions.pop_front();

		// If it's the same position as the object is already in, skip it
		if (Wm5::Mathd::FAbs(MoveAmount) <= 0.000100)
			continue;

		MoveAmount += 2 * -m_kBufferDistance;		// Pull it a little *forward* to force a collision, which will cause geometry to resolve
		MoveSelectedObject(MoveAmount * SnapDirection);

		auto CollisionExists = CheckForCollision();
		if (!CollisionExists)
		{
//printf("Moved forward....    no collision, by %f\n", MoveAmount);
			return true;
		}
		else
		{
...
//DontCare = 0;
			...
			if (ResolvedCollision)
			{
//printf("Moved forward....    by %f but Collision, resolved with %f\n", MoveAmount, DontCare);
				return true;
			}
		}
...
bool Slide::FindFirstValidPositionB(MultiRange<float> & Positions, const Wm5::Vector3d SnapDirection, double & OutputPosition, bool & OutputHaveSkippedStartingPosition)
{
	OutputPosition = 0;
	OutputHaveSkippedStartingPosition = false;
	...
	while (!Positions.empty())
	{
		double MoveAmount = 100 * static_cast<double>(Positions.front().first);		// Use lower bound
		OutputPosition = MoveAmount;
		Positions.pop_front();

		// If it's the same position as the object is already in, skip it
		if (Wm5::Mathd::FAbs(OutputPosition) <= 0.003000)
		{
			OutputHaveSkippedStartingPosition = true;
			continue;
		}
		...
		{
			auto ResolvedCollision = TryResolveCollisionGeometry(SnapDirection, OutputPosition);
//printf("Moved back-2nl.....      TryResolveCollisionGeometry diff %f\n", Wm5::Mathd::FAbs(OutputPosition - MoveAmount));
			if (ResolvedCollision)
			{
				// If it's the same position as the object is already in, skip it
				if (Wm5::Mathd::FAbs(OutputPosition) <= 0.000500)
				{
					OutputHaveSkippedStartingPosition = true;
					continue;
				}
...
// Snap front, assuming collision exists
bool Slide::NewSnapB(SnapMode SnapMode, StartingPosition StartingPosition)
{
	...
	if (Front == SnapMode)
		SnapDirection = -m_CursorRayDirection;		// Assuming m_CursorRayDirection is normalized
	//SnapDirection = m_CursorRayDirection;		// Using consistent direction of away from camera as the along-ray-position coordinate system
...

	{
		...
		for (uint8 DepthLayer = 0; DepthLayer <= MaxLayersToPeel; ++DepthLayer)
		{
			// Set target FBO
			glBindFramebuffer(GL_FRAMEBUFFER, MySlideTools->m_DepthPeelFboId[DepthLayer % 2]);
			//glDrawBuffer(GL_COLOR_ATTACHMENT0);
			...
			if (0 == DepthLayer)
			{
				...
				// Render the selected model
				if (GetDebugSwitch(DebugSwitch::DEBUG_SWITCH_SKIP_INNER_SELOBJ_SNAP_POSITIONS))
					MyScene.Render(Scene::SelectedObjectClosestFrontFace, this);		// This skips inner snap points of the selected object
				else
					MyScene.Render(Scene::SelectedObjectFurthestFrontFace, this);
				...
				if (Front == SnapMode)
				{
					...
					for (uint32 nPixel = 0; nPixel < nCustomViewportWidth * nCustomViewportHeight; ++nPixel)
					{
						...
						if ((sr+sg) != 0 && (sr2+sg2) != 0 && (r+g) != 0)
						{
							...
							if (dDepthDiff >= 0.0)
							{
								auto Pair = std::pair<float, float>(dDepthDiff - SelectedObjectThickness, dDepthDiff);
								mr.insert(Pair);
							}
						}
					}
					...
					{
						auto FoundValidPosition = FindFirstValidPosition(mr, SnapDirection);
...
//SlidingNormal = MyScene.GetSlidingPlane().Normal;
	...
	printf("SelObj.Z() = %.20f (%f, %f) Snap-Front\n", GetSelectedObject().GetPosition().Z(), GetSelectedObject().GetPosition().X(), GetSelectedObject().GetPosition().Y());
	return SuccessfulSnap;
}

bool Slide::EqualPositions(const Wm5::Vector3d Position0, const Wm5::Vector3d Position1)
{
	return (Position1 - Position0).SquaredLength() <= (0.000001 * m_kBufferDistance * m_kBufferDistance);
}

bool Slide::NewSnapC(SnapMode SnapMode)
{
	bool SuccessfulSnap = false;

	Wm5::Vector3d SnapDirection;
	if (BackNextLevel == SnapMode)
		SnapDirection = m_CursorRayDirection;		// Assuming m_CursorRayDirection is normalized
...

	{
		...
		for (uint8 DepthLayer = 0; DepthLayer <= MaxLayersToPeel; ++DepthLayer)
		{
			...
			{
				if (BackNextLevel == SnapMode) {
					MyScene.Render(Scene::SelectedObjectClosestFrontFace, this);
				...
				// Render the selected model
				if (GetDebugSwitch(DebugSwitch::DEBUG_SWITCH_SKIP_INNER_SELOBJ_SNAP_POSITIONS))
					MyScene.Render(Scene::SelectedObjectFurthestBackFace, this);		// This skips inner snap points of the selected object
				else
					MyScene.Render(Scene::SelectedObjectClosestBackFace, this);
				...
				if (BackNextLevel == SnapMode)
				{
					...
					for (uint32 nPixel = 0; nPixel < nCustomViewportWidth * nCustomViewportHeight; ++nPixel)
					{
						...
						if ((sr+sg) != 0 && (sr2+sg2) != 0 && (r+g) != 0)
						{
							float dDepthDiff = d - sd;
							///assert(sd2 <= sd);
							float SelectedObjectThickness = sd - sd2;		// Should be a non-negative number >= 0
							//printf("SelectedObjectThickness %f\n", SelectedObjectThickness);

							if (dDepthDiff >= 0.0)
							{
								auto Pair = std::pair<float, float>(dDepthDiff, dDepthDiff + SelectedObjectThickness);
								mr.insert(Pair);
							}
						}
					}
					...
					{
						double Position;
						bool HaveSkippedStartingPosition;
						auto FoundValidPosition = FindFirstValidPositionB(mr, SnapDirection, Position, HaveSkippedStartingPosition);
						if (FoundValidPosition)
						{
							printf("FindFirstValidPositionB returned position %f\n", Position);
							...
							// Check if we've completely peeled the space between the starting and original best position, and break out if so
							if (!HaveSkippedStartingPosition && Position >= BestPosition)
							{
								printf("Breaking out on DepthLayer %d\n", DepthLayer);
								break;
							}
						}
					}
		...
		if (-1 != BestPosition)
		{
			MoveAmount = BestPosition;
			printf(" >>>> USING BestPosition %f\n", BestPosition);

			// DEBUG: I shouldn't be doing this again here; it's already been done earlier. I should save the earlier results and restore them here.

			MoveAmount += 2 * m_kBufferDistance;		// Push it a little further *back* to force a collision, which will cause geometry to resolve
			MoveSelectedObject(MoveAmount * SnapDirection);

			bool CollisionExists = CheckForCollision();
			printf("CollisionExists = %d at new place\n", CollisionExists);
		...
		if (-1 == BestPosition)
		{
			// DEBUG: Just clearing the collision info, assuming I started without collision. It should be rewritten to simply return to original state (whatever that might be), probably by changing the function to work with temporary collision info objects.
			for (std::vector<SceneObject *>::iterator it0 = MyScene.m_Objects.begin(); it0 != MyScene.m_Objects.end(); ++it0)
				(*it0)->m_oIntersectingTris.clear();
			m_IntersectingPairs.clear();
		}
...
//SlidingNormal = MyScene.GetSlidingPlane().Normal;
	...
	printf("SelObj.Z() = %.17f (%f, %f) Snap-Back-to-Next-Level\n", GetSelectedObject().GetPosition().Z(), GetSelectedObject().GetPosition().X(), GetSelectedObject().GetPosition().Y());
	return SuccessfulSnap;
}
...
double Slide::SnapGeometry2(Wm5::Vector3d SnapDirection)
{
	...
	// Geometry resolution
	{
		...
		MoveAmount = -MaxResolutionDistance;
	}
...
//printf("preObj.Z() = %.20f (%f, %f)\nSnapGeome(Front): ", (MyScene.GetSelectedObject().GetPosition() + MoveVector).Z(), (MyScene.GetSelectedObject().GetPosition() + MoveVector).X(), (MyScene.GetSelectedObject().GetPosition() + MoveVector).Y());
	{
		...
		if (0 != m_SlidingObject) {
			Wm5::Vector3d SlidingNormal = GetSlidingPlane(MoveAmount * SnapDirection).Normal;		// For GetSlidingPlane() to work properly, objects have to be in proper place
//SlidingNormal = MyScene.GetSlidingPlane().Normal;
			/// Target = m_kBufferDistance / cos(Theta)
			double FAbsCosTheta = Wm5::Mathd::FAbs(SnapDirection.Dot(SlidingNormal));
			if (FAbsCosTheta < 0.000001)
...
bool Slide::CheckForVisibility()
{
	...
	{
		MyScene.Render(Scene::SelectedObjectVisibilityQuery, this);
