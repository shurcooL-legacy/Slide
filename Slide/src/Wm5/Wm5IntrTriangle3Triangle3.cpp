    ...
    ContactSide& side, Configuration& cfg0, Configuration& cfg1)
{
    if (side == CS_RIGHT) // tri1 to the right of tri0
    {
        ...
        else if (cfg0.mMap == M12)
        {
            if (cfg1.mMap == M21)
            {
                ...
                GetEdgeEdgeIntersection(tri0.V[1], tri0.V[2],tri1.V[0],
                    tri1.V[1]);
            }
            else // cfg1.mMap == m3
            {
                // uedge-vface intersection
                GetEdgeFaceIntersection(tri0.V[1], tri0.V[2], tri1);
            }
        }
        else // cfg0.mMap == M3
        {
            if (cfg1.mMap == M21)
            {
                ...
                GetEdgeFaceIntersection(tri1.V[0], tri1.V[1], tri0);
            } 
            else // cfg1.mMap == M3
            {
    ...
    else if (side == CS_LEFT) // tri1 to the left of tri0
    {
        ...
        else if (cfg1.mMap == M12)
        {
            ...
            {
                // edge0-edge1 intersection
                GetEdgeEdgeIntersection(tri0.V[0], tri0.V[1], tri1.V[1],
                    tri1.V[2]);
            }
            else // cfg0.mMap == M3
            {
                // edge1-face0 intersection
                GetEdgeFaceIntersection(tri1.V[1], tri1.V[2], tri0);
            }
        }
        else // cfg1.mMap == M3
        {
            if (cfg0.mMap == M21)
            {
                ...
                GetEdgeFaceIntersection(tri0.V[0], tri0.V[1], tri1);
            } 
            else // cfg0.mMap == M
            {
                // face0-face1 intersection
    ...
    const Vector3<Real>& V0, const Vector3<Real>& V1)
{
    ...
    assertion((Real)0 <= s && s <= (Real)1, "Unexpected s\n");
    ...
    const Triangle3<Real>& tri)
{
    ...
    IntrSegment2Triangle2<Real> calc(projSeg, projTri);
    bool result = calc.Find();
    assertion(result, "There must be an intersection\n");
    WM5_UNUSED(result);
    mQuantity = calc.GetQuantity();
    for (int i = 0; i < mQuantity; ++i)
    {
