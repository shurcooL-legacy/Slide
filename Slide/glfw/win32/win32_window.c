				...
				if (GetTouchInputInfo((HTOUCHINPUT)lParam, cInputs, pInputs, sizeof(TOUCHINPUT)))
				{
					...
					for (Input = 0; Input < cInputs; ++Input)
					{
						...
						if (TOUCHEVENTF_PRIMARY != (pInputs[Input].dwFlags & TOUCHEVENTF_PRIMARY))
						{
							...
							//if (TOUCHEVENTF_MOVE == (pInputs[Input].dwFlags & TOUCHEVENTF_MOVE))
								if( _glfwWin.touchPosCallback )
								{
							...
							if (TOUCHEVENTF_DOWN == (pInputs[Input].dwFlags & TOUCHEVENTF_DOWN))
								if( _glfwWin.touchButtonCallback )
								{
									_glfwWin.touchButtonCallback( 0, GLFW_PRESS );
								}

							if (TOUCHEVENTF_UP == (pInputs[Input].dwFlags & TOUCHEVENTF_UP))
								if( _glfwWin.touchButtonCallback )
								{
									_glfwWin.touchButtonCallback( 0, GLFW_RELEASE );
								}
